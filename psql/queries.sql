...
-- reset autoincrement (PostgreSQL uses sequences)
...
ALTER SEQUENCE tests_sessions_id_seq RESTART WITH 1;
ALTER SEQUENCE questions_options_id_seq RESTART WITH 1;
...
VALUES
    ('sql', 'stuctured query language test', '00:30', 'choose correct option for questions', 'sql'),
...
-- Add a new proctoring sessions for proctor when start proctoring test sessions (assuming its manual)
...
-- some updates
UPDATE "tests_sessions" SET "status" = 'completed' WHERE "id" = 1;
UPDATE "tests_sessions" SET "status" = 'ended' WHERE "id" = 2;
